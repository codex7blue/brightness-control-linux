...
1. Pastikan paket **libinput** sudah terinstall:
   ```
...
7. Pastikan user anda masuk dalam grup <mark>video</mark>:
   ...
   ```
8. Pastikan juga daemon **udevd** sudah terinstall, jika belum install dan jalankan service:
   ```
...
9. Buat udev rule di <mark>/etc/udev/rules.d/</mark>
   ...
   - Kami juga sudah menyediakan file <mark>90-brightness.rules</mark> dan anda bisa memindahkannya.
...
```
    ...
    ```
    - Kami juga telah menyediakan file run dan finish yang mengeksekusi perintah script <mark>libinput-brightness.sh</mark> serta menyimpan dan mengembalikan level kecerahan layar setelah shutdown/reboot.
...
Catatan:
- Script **brightness.sh** menggunakan <mark>sysfs</mark> di <mark>/sys/class/backlight/*</mark>
...
[ ] 1. Pastikan script **/usr/local/bin/brightness.sh** bisa diakses dan executable:
```
sudo chmod +x /usr/local/bin/brightness.sh
```
[ ] 2. Pastikan script **/usr/local/bin/libinput-brightness.sh** sudah:
